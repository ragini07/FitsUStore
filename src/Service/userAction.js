...
export const addToCart = async (dispatchUserData , token , product) => {
    try {
       const {data , status} = await axios.post('/api/user/cart',{
        ...
        console.log("data from server add to cart ",data)
    }catch(error){
        console.log(error)
    }
}
export const removeFromCart = async(dispatchUserData , token , product) => {
    ...
    try{
       const {data , status } = await axios.delete(`/api/user/cart/${product._id}`,{
        ...
        console.log("data from server remove to cart ",data , status)
        if(status === 200){
...
export const changeQtyInCart = async(dispatchUserData , token , product , type) => {

    try{
       const {data , status } = await axios.post(`/api/user/cart/${product._id}`,{
        action: {
          type: type
        }
      },{
        ...
        console.log("data from server chnage to cart ",data)
        if(status === 200){
            if(type === 'increment')
              dispatchUserData({type : "INCREMENT_QUANTITY" , payload : product})
            else
                dispatchUserData({type : "DECREMENT_QUANTITY" , payload : product})
        }
    }catch(error){
        console.log(error)
    }
...
export const addToWishList = async (dispatchUserData , token , product) => {
    try {
       const {data , status} = await axios.post('/api/user/wishlist',{
        ...
        console.log("data from server add to wishlist ",data)
    }catch(error){
        console.log(error)
    }
}
export const removeFromWishList = async(dispatchUserData , token , product) => {
    console.log(product._id)
    try{
       const {data , status } = await axios.delete(`/api/user/wishlist/${product._id}`,{
            headers : {
                authorization : token
            }
        })
        console.log("data from server remove from wishlist ",data , status)
        if(status === 200){
