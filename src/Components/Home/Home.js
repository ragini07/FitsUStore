  ...
  return (
    <>
     <header className="showcase">
            ...
            <button class="btn"><Link to= '/products' className="link-btn"> Shop Now <i class="fa fa-chevron-right"></i></Link></button>
        </header>
         ...
         {   homeCategory.map(({_id,imageURL,categoryName}) => {
                               return ( 
                                <div 
                                    class="home-card" 
                                    key={_id}
                                    onClick={() => {dispatchFilterState({type : "BY_CATEGORY" , payload : categoryName})
                                             navigate('/products')} } >
