...
export const filterByCategory = (filteredByRatingData, category) => {
  ...
  for (let c in category) {
    if (category[c]) categorySelected = [...categorySelected, c];
  }
  if (categorySelected.length > 0)
    return filteredByRatingData.filter((item) =>
      categorySelected.some((c) => c === item.category)
    );
  return filteredByRatingData;
};
export const filterByStock = (filteredByCategoryData, showOutOfStock) => {
  if (showOutOfStock) return filteredByCategoryData;
...
export const filterBySearchQuery = (filteredByDeliveryData, searchQuery) => {
  if (searchQuery)
    return filteredByDeliveryData.filter((item) =>
